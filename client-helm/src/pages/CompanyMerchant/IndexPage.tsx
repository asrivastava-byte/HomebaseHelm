...
export function CompanyMerchantIndexPage() {
  const [q, setQ] = useState("");
  const debounced = useDebounced(q, 250);

  const { data, isFetching } = useQuery({
    queryKey: ["companies", "search", debounced],
    queryFn: () => companiesApi.search(debounced),
  ...
  return (
    <Box>
      ...
      <Box mt={2}>
        ...
        <List>
          {results.map((r: CompanySummary) => (
            <ListItemButton key={r.id} component={RouterLink} to={`/companies/${r.id}`}>
